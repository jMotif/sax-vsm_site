...
weight: 4
pagekind: "reading"
summary: "An overview of time series discretization via sliding window."
labels:
  - algorithm
---
## Definitions
...
#### Subsequence
Subsequence \(C\) of time series \(T\) is a contiguous sampling \(t_{p},\dots,t_{p+n-1}\) of points of length \(n \ll m\) where \(p\) is an arbitrary position, such that \( 1 \leq p \leq m - n + 1\). Typically subsequences are extracted from a time series with the use of a sliding window:
...
Continue to the [SAX discretization numerosity reduction]({{< ref "/algorithm/numerosityreduction" >}}) module...
