...
---
This module walks through classifying time series with the SAX-VSM Java implementation — from building the jar to interpreting *why* the classifier decided the way it did. The transcripts below were captured with `sax-vsm` 2.0.1; the classifier is unchanged in 2.1.0 (that release rewrote the clustering package and added degenerate-input guards), and the classification and sampler results below were re-verified against 2.1.0.

## 1. Get the code

SAX-VSM ships on Maven Central as `net.seninp:sax-vsm:2.1.0`. For command-line use, build the self-contained jar from [the repository]({{< param github >}}):
...
```text
$ java -cp "target/sax-vsm-2.1.0-jar-with-dependencies.jar" net.seninp.jmotif.direct.SAXVSMPatternExplorer \
